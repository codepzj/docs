...
GEN 支持像 GORM 一样的处理关联的方式。 关系(属于 To/HasOne/HasMany2Many) 重新使用 GORM 的标签。 支持关联已经存在的 model，或者关联数据库中的表两种方式。
...
如果关联的 mode 已经存在， 可以通过`gen.FieldRelateModel` 设置关联。
...
支持关联已经存在的 model，或者关联数据库中的表两种方式。

### Preload

GEN 允许使用 `Preload`通过多个 SQL 来直接加载关联表数据, 例如：
